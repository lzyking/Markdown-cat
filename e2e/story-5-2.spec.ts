...
test.describe('Story 5.2：响应式预览自适应', () => {
  ...
  test('拖动 splitter 与窗口 resize 后应更新预览响应式布局标记', async ({ page }) => {
    ...
    await expect(preview).toHaveAttribute('data-preview-layout', 'regular')

    const initialWidth = Number(await preview.getAttribute('data-preview-width'))
    expect(initialWidth).toBeGreaterThan(420)

    await dragSplitterTo(page, 900)
    await expect(preview).toHaveAttribute('data-preview-layout', 'compact')

    const compactWidth = Number(await preview.getAttribute('data-preview-width'))
    expect(compactWidth).toBeLessThan(initialWidth)

    await page.setViewportSize({ width: 1600, height: 700 })
    await page.waitForTimeout(100)

    await expect(preview).toHaveAttribute('data-preview-layout', 'wide')

    const wideWidth = Number(await preview.getAttribute('data-preview-width'))
