...
## Epic 5: v0.2.0 新版本功能增强

### Story 5.1: 启动自动恢复上次文件 & 默认保存路径升级

As a 用户，
I want 应用启动时自动恢复上次打开的文件，且默认保存目录位于 `My Markdown`，
So that 我可以无缝延续工作，且归档路径统一规范。

**Acceptance Criteria:**

**Given** 用户上一次使用应用编辑或保存过文件
**When** 再次启动应用
**Then** 应用自动从配置中读取 `lastOpenedFile` 并自动加载该文件内容
**And** 窗口标题显示上次打开的文件名

**Given** 上次打开的文件不存在、已被移动或损坏
**When** 应用启动
**Then** 应用自动回退生成并显示一个新的空白文档
**And** 状态栏给出优雅提示，不阻塞软件正常使用

**Given** 首次使用应用或未配置自定义保存路径
**When** 系统寻找默认保存目录
**Then** macOS 和 Windows 系统的基准保存路径统一为 `~/Documents/My Markdown`
**And** 若该目录不存在，系统在启动/保存时自动创建该文件夹

### Story 5.2: 重新设计【文件】菜单支持【打开】与【另存为】

As a 用户，
I want 在菜单栏【文件】中找到【打开】与【另存为】入口，
So that 我可以随时打开任意本地 Markdown 文件或将当前文件存到任意位置。

**Acceptance Criteria:**

**Given** 用户点击顶部菜单栏【文件】
**When** 展开下拉菜单
**Then** 显示【打开文件 (Open)...】、【另存为 (Save As)...】和【设置默认保存路径...】菜单项

**Given** 用户点击【打开文件 (Open)...】
**When** 系统弹出文件选择器并选择一个 `.md` 或 `.txt` 文件
**Then** 应用读取并载入该文件内容至编辑器
**And** 将该文件路径记录为最近打开文件

**Given** 用户点击【另存为 (Save As)...】
**When** 系统弹出另存为对话框并选择新的保存路径
**Then** 当前内容写入选定路径
**And** 应用绑定该新路径并更新标题栏与保存状态

### Story 5.3: 外部 Markdown 文件拖拽 (Drag & Drop) 识别与导入

As a 用户，
I want 直接将本地的 `.md` 文件拖拽到 Markdown Cat 界面内，
So that 我能以最方便的方式快速查看和编辑该文件。

**Acceptance Criteria:**

**Given** 用户拖拽一个 `.md` 或 `.txt` 文件到应用主窗口
**When** 在窗口上方释放鼠标 (Drop)
**Then** 应用接收拖拽文件事件并读取文件路径与内容
**And** 编辑器与预览区同步更新为该文件内容
**And** 自动更新最近打开文件记录

### Story 5.4: 编辑器光标焦点提示与 `/` 快捷插入 Markdown 语法浮窗菜单

As a 用户，
I want 在编辑器中看到清晰的光标与 `/` 快捷语法提示，并能通过输入 `/` 呼出快捷插入菜单，
So that 我不需要手动输入复杂的 Markdown 符号就能快速排版。

**Acceptance Criteria:**

**Given** 编辑器刚挂载或处于空白文档状态
**When** 用户未输入内容
**Then** 编辑器呈现光标闪烁，并显示占位提示语“按 / 键快速插入 markdown 格式”

**Given** 用户在编辑器中按下 `/` 键
**When** 按键触发
**Then** 在光标下方弹出一个快捷插入面板 (Slash Menu)
**And** 面板中包含 H1/H2/H3 标题、加粗、斜体、引用、代码块、无序/有序列表等选项

**Given** 快捷插入面板已显示
**When** 用户按下 `↑` `↓` 切换选项并按 `Enter`（或使用鼠标点击某一项）
**Then** 自动将面板中的 Markdown 语法模板替换插入到光标处
**And** 自动将光标定位到待输入正文位置并关闭面板
